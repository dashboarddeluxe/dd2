...
<div class="grid grid-cols-1 sm:grid-cols-1 lg:grid-cols-1 gap-4 mb-4">
  ...
  <img src="{{ $group->cover_image }}" alt="{{ $group->title }} cover image" class="h-64 w-full object-cover
            object-top">
            @else
            ...
            <div class="px-4 py-2 bg-gray-800">
                <h2 class="text-xl text-white">
                    {{--
    <a href="{{ $group->getUrl() }}" title="Read {{ $group->title }}" class="text-white font-bold">
                    --}}
                    {{ $group->title }}
                    {{--
    </a>
                ...
                <p class="text-gray-700 text-base">
                    ...
                    <div class="grid grid-cols-1 xl:grid-cols-2">
                        ...
                        <div
                            ...
                            @if ($loop->first)
                                @if ($link[1] === "heading")
                                    class="xl:col-span-2 border-b-2 border-gray-400 my-2" >
...
@foreach ($posts->where('featured', true)->take(6) as $group)
  ...
  <img src="{{ $group->cover_image }}" alt="{{ $group->title }} cover image" class="h-64 w-full object-cover
    ...
    <div class="px-6 py-4 bg-blue-100">
        <h2 class="font-bold text-2xl mb-2"><a href="{{ $group->getUrl() }}" title="Read {{ $group->title }}"
                class="text-gray-900 font-extrabold">{{ $group->title }}</a></h2>
